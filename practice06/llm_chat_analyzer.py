"""
Practice06: 聊天记录总结 + 关键信息提取 + 聊天历史搜索 + AnythingLLM 查询 + 技能系统
基于 llm_chat_analyzer.py，新增：
1. 技能系统，支持读取和使用技能
2. list_available_skills 函数，读取技能列表
3. load_skill_content 函数，加载技能内容
4. 自动检测用户请求并调用相应技能
"""
...
def list_available_skills(skills_dir=None):
    ...
    Args:
        skills_dir: 技能目录路径，默认为 .agents/skills
...
def load_skill_content(skill_name, skills_dir=None):
    """
    加载技能内容，返回技能的正文部分（YAML front matter 之后的内容）
...
def is_skill_request(user_input, skills):
    """
    判断用户输入是否需要使用技能
    ...
    for skill in skills:
        ...
        # 简单的关键词匹配
        if skill_name in user_input_lower:
            return True, skill['name']
    
    # 特殊处理 notice 技能
    notice_keywords = ['通知', '撰写通知', '修改通知', '润色通知']
...
def print_header():
    ...
    """
    print("\n" + "=" * 60)
    print("Practice06: 聊天记录总结 + 关键信息提取 + 聊天历史搜索 + AnythingLLM 查询 + 技能系统")
    print("=" * 60)
    print("\n提示:")
    ...
    print("   - 提到 '通知'、'撰写通知' 等关键词时使用 notice 技能")
...
def main():
    ...
    system_prompt = f"""你是一个乐于助人的助手，能够回答各种问题。
...
当用户的请求需要使用技能时，请根据技能列表选择合适的技能，并使用相应的工具加载技能内容。"""
    ...
    try:
        while True:
            try:
                ...
                # 检查是否需要使用技能
                is_skill, skill_name = is_skill_request(user_input, skills)
                if is_skill:
                    print(f"\n" + "=" * 60)
                    print(f"检测到需要使用技能: {skill_name}")
                    print("=" * 60)
                    ...
                    # 将技能内容添加到系统提示
                    skill_system_prompt = f"""现在需要使用 {skill_name} 技能处理用户请求。
技能内容：
{skill_content}

请根据技能内容处理用户请求：{user_input}"""
                    
                    # 构建包含技能内容的消息
                    skill_messages = conversation_history + [
                    ...
                    for chunk in call_llm_api_stream(env_vars, skill_messages):
